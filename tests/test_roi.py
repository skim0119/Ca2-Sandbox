...
def test_handle_rois_existing_json(temp_dir, sample_image, sample_rois):
    ...
    with patch("ca2roi.roi.draw_roi_images") as mock_draw:
        result = handle_rois(
            sample_image, roi_json_path, roi_img_path, roi_img_label_path
        )

        assert result == sample_rois
        mock_draw.assert_called_once_with(
            sample_image, sample_rois, roi_img_path, roi_img_label_path
        )
...
def test_draw_roi_images(
    mock_fromarray, mock_font, mock_draw, temp_dir, sample_image, sample_rois
):
